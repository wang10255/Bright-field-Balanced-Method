...
elseif strcmpi(Flag_AperShape,'ring')
    [Theta,R] = cart2pol(x,y);
    Rmax = max(R(~isnan(R)));
    Aperture = x.^2+y.^2<0.8*Rmax*8e-5;
    Rmax_CCD = Rmax/2; % Note: Light field is larger than CCD, so Rmax/2.
    %     Aperture(y>0.4*Rmax_CCD) = 0;
...
elseif strcmpi(Flag_AperShape,'telescope')
    ...
    %     Aperture(y>0.4*Rmax_CCD) = 0;
    Aperture(R<0.2*Rmax_CCD) = 0;
    Aperture(x>-3e-6 & x<5e-6) = 0;
    Aperture(y>-3e-6 & y<5e-6) = 0;
...
elseif strcmpi(Flag_AperShape,'telescope-')
    ...
    Aperture(sqrt((x+1.35e-4).^2+(y+1e-4).^2)<2e-5) = 0;
